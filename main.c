...
const char* tankId = "3681"; //STB-1

const char* timeUrl = "https://api.wotblitz.ru/wotb/account/info/?application_id=%s&account_id=%s&fields=last_battle_time";
const char* dataUrl = "https://api.wotblitz.ru/wotb/account/tankstats/?application_id=%s&account_id=%s&tank_id=%s";
...
int main() {
  ...
  sprintf(url, dataUrl, appId, accountId, tankId);
  ...
  printf("\x1B[2J\x1B[H");
  printf(" num | damage | hitrate | winrate | survival | frags | spots |  xp  | M |  time\n");
  for (;;) {
    curl_easy_perform(timeHandle);
    if (newTime != lastBattleTime) {
      ...
      printf("\x1B[2K\r");
      printf("%4i | %6i | %6.2f%% | %7s | %8s | %5d | %5d | %4d | %c | %5d\n", 
             ...
             battleStats.battleTime);
      printf(" avg |%7.1f | %6.2f%% | %6.2f%% | %7.2f%% | %5.2f | %5.2f | %4d | %c | %5.2f",
             ...
             (float)sessionStats.battleTime / sessionStats.battles);
      fflush(stdout);
...
size_t dataParse(void *buffer, size_t size, size_t nmemb, Stats *pStats) {
  json_object *obj = json_tokener_parse(buffer);
  json_object *data, *me, *all;
  json_object_object_get_ex(obj, "data", &data);
  json_object_object_get_ex(data, accountId, &me);
  json_object_object_get_ex(me, "all", &all);
  json_object *damage_dealt, *shots, *hits, *battles, *survived_battles, *wins, *losses, *frags, *spotted, *xp, *battle_life_time;
  ...
  json_object_object_get_ex(me, "battle_life_time", &battle_life_time);
  ...
  pStats->battleTime = json_object_get_int(battle_life_time);
  return size * nmemb;
}
...
void calculateStatsDifference(Stats *pMinuendStats, Stats *pSubtrahendStats, Stats *pResultStats) {
  ...
  pResultStats->xp = pMinuendStats->xp - pSubtrahendStats->xp;
  ...
  pResultStats->battleTime = pMinuendStats->battleTime - pSubtrahendStats->battleTime;
}
